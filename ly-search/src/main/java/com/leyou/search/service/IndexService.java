...
import com.leyou.search.utils.PinYin4j;
...
public class IndexService {
    ...
    public Goods buildGoods(SpuBO spu){
        ...
        String[] split = spu.getTitle().split(" ");

        String first =null;
        for (String s : split) {
            first= first +" "+PinYin4j.getPinYinStringHeadChar(s);
        }
        String pinYin = PinYin4j.getPinYin(all);


        //sku集合
        List<Sku> skus = this.goodsClient.findSkuById(id);
        //处理sku
        //把商品价格取出单独存放，便于展示
        ...
        //spuDetail
        SpuDetail spuDetail = this.goodsClient.findSpuById(id);
        //查询分类对应的规格参数
